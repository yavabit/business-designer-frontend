        ...
        if (result.data) {
          ...
          if (page === 1 || reset) {
            ...
            const uniqueNewProcess = newProcesses.filter(
              (newProcess) =>
                !allProcesses.some((existing) => existing.id === newProcess.id)
            );
            setAllProcesses((prev) => [...prev, ...uniqueNewProcess]);
          }
  ...
  return (
    ...
    >
      <Flex gap="middle" vertical justify="space-evenly">
        {isLoading && currentPage === 1 && allProcesses.length === 0 && (
        ...
        {isError && (
          <Flex vertical gap={12} align="center" style={{ padding: "20px" }}>
            <p>Ошибка при загрузке процессов!</p>
            <Button onClick={() => loadPage(1, true)}>Попробовать снова</Button>
          </Flex>
        )}
        ...
        {!isLoading &&
          ...
          searchString === "" && (
            <Flex vertical gap={12} align="center" style={{ padding: "20px" }}>
              ...
              <Button onClick={() => dispatch(setProcessCreationModal(true))}>
                Создать?
              </Button>
            </Flex>
          )}
            ...
            {allProcesses.map((item) => (
              <Card
                ...
                actions={[
                  ...
                  <div onClick={() => navigate(`/process/${item.id}`)}>
        ...
        {isFetching && currentPage > 1 && (
          <Flex justify="center" style={{ padding: "20px" }} ref={loadingRef}>
            <Spin size="large" />
          </Flex>
        )}

        {!hasMore && allProcesses.length > 0 && (
          <Flex justify="center" style={{ padding: "20px", color: "#666" }}>
            <p>Все процессы загружены ({allProcesses.length} всего)</p>
          </Flex>
        )}
